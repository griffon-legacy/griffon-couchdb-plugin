...
giving you access to a `org.jcouchdb.db.Database` object, with which you'll be able
...
This method is also accessible to any component through the singleton `griffon.plugins.couchdb.CouchdbConnector`.
...
The `withCouchdb()` dynamic method will be added to controllers by default. You can
 ...
 * ConfigureCouchdbJSONParser[config, databaseName, parser] - triggered before the JSONConfig is built and applied to the database
 * CouchdbConnectEnd[databaseName, database] - triggered after connecting to the database
 * CouchdbDisconnectStart[config, databaseName, database] - triggered before disconnecting from the database
...
A trivial sample application can be found at [https://github.com/aalmiray/griffon_sample_apps/tree/master/persistence/couchdb][2]

Testing
-------
The `withCouchdb()` dynamic method will not be automatically injected during unit testing, because addons are simply not initialized
for this kind of tests. However you can use `CouchdbEnhancer.enhance(metaClassInstance, couchdbProviderInstance)` where 
`couchdbProviderInstance` is of type `griffon.plugins.couchdb.CouchdbProvider`. The contract for this interface looks like this

    public interface CouchdbProvider {
        Object withCouchdb(Closure closure);
        Object withCouchdb(String databaseName, Closure closure);
        <T> T withCouchdb(CallableWithArgs<T> callable);
        <T> T withCouchdb(String databaseName, CallableWithArgs<T> callable);
    }
...
fails regardless of the arguments it receives
    ...
    class MyCouchdbProvider implements CouchdbProvider {
        Object withCouchdb(String databaseName = 'default', Closure closure) { null }
        public <T> T withCouchdb(String databaseName = 'default', CallableWithArgs<T> callable) { null }
    }
